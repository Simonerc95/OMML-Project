...
class RBF:
    ...
    def two_block(self, method='bfgs', num_iter=100, maxiter=1000, th=10, disp=False):
        # step 1
        self.C = np.random.normal(loc=0., scale= 1, size=(self.N, self.n))  # N x n 
        self.v = np.random.normal(loc=0., scale= 1, size=(self.N, 1))  # N x 1 
        t = time.time()
        ...
        for i in range(num_iter):
            ...
            opt = minimize(self._optimize, self.v.flatten(), jac=self.grad, method='bfgs', options=dict(maxiter=1))
            
            self.v = opt.x.reshape(self.N,1)
                
            C = self.C
            opt_2 = minimize(self._optimize_2, C.flatten(), method='bfgs', options=dict(maxiter=1))
            ...
            if R_old-R_val > 0:
                ...
                tolerance += 1
                if tolerance > th:
                    print(f'tolerance exceded, num_iter: {i}')
                    break
            print(f'step: {i}')
